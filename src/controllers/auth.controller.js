...
export const loginGet = (req, res) => {
  if (res.locals.isLoggedIn) return res.redirect("/dashboard");
  res.render("auth/login", {
    title: "Login",
    layout: "layouts/auth_layout",
    firebasePublic: res.locals.FIREBASE_CONFIG_PUBLIC, // <— AQUI
  });
};
...
export const logout = async (req, res) => {
  ...
  const isLocalhost = req.hostname === 'localhost' || req.hostname === '127.0.0.1';
  res.clearCookie('session', { httpOnly: true, sameSite: 'Lax', secure: !isLocalhost });
